...
// This is a vendored third-party crate built via `[patch]`. Cargo's automatic
// `--cap-lints allow` only covers registry/git deps, not path patches, so the
// upstream 2015-era style (e.g. `extern` without an explicit ABI) would
// otherwise spam the build. Silence it; we don't develop this code, only the
// one-line uninhabited-static fix above.
#![allow(warnings)]
...
// Vendored fix: upstream declared `enum Class {}`, an uninhabited type, then
// an `extern static` of it — which rustc is phasing out (rust-lang/rust#74840)
// and reports as a future-incompatibility. `Class` is only ever used as an
// opaque pointer target (`*const Class`) and we only take the static's
// address, so an inhabited zero-sized opaque type is ABI-identical and clears
// the lint.
#[repr(C)]
...
// Upstream's `#[cfg(test)] mod tests` (which relied on the dropped
// `objc_test_utils` C helper) is omitted from this vendored copy; we build it
// only as a dependency, never run its own test suite.
