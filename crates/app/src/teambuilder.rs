...
impl TeamBuilderView {
    fn new(project_dir: Option<PathBuf>, window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        let mut roles = crate::relay::role_list();
        if roles.is_empty() {
            roles = ["supervisor", "worker", "frontend", "backend", "reviewer", "devops", "qa"]
                .iter()
                .map(|s| s.to_string())
                .collect();
        }
    ...
    /// Collect the form into a [`TeamSpec`], save it via relay, and on success
    /// refresh the AI menu and close. Errors surface in the status line.
    fn commit(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        match crate::relay::save_team(&spec, user, self.project_dir.as_deref()) {
            Ok(saved) => {
...
impl Render for TeamBuilderView {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let footer = div()
            ...
            .when(show_save, |d| {
                d.child(
                    Button::new("team-save", "Save Team")
                        .variant(Variant::Filled)
                        .on_click(move |_ev, window, app| {
